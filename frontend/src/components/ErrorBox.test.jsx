...
describe('ErrorBox', () => {
  ...
  it('applies error styling classes by default', () => {
    ...
    expect(box.className).toContain('text-red-700');
    expect(box.className).toContain('border-red-200');
  });
  ...
  it('applies success styling classes', () => {
    ...
    expect(box.className).toContain('text-green-700');
    expect(box.className).toContain('border-green-200');
  });
  ...
  it('applies info styling classes', () => {
    ...
    const box = container.firstChild;
    expect(box.className).toContain('bg-sky-50');
    expect(box.className).toContain('text-sky-700');
  ...
  it('falls back to error styling for unknown variant', () => {
    const { container } = render(<ErrorBox variant="warning" message="Warning" />);
  ...
  it('applies extra className alongside base classes', () => {
    const { container } = render(
      <ErrorBox message="Error" className="my-custom-class" />,
    );
