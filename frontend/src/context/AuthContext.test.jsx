...
// ---------------------------------------------------------------------------
var mockFetchCurrentUser;
var mockLoginRequest;
var mockLogoutRequest;
...
mockFetchCurrentUser = api.fetchCurrentUser;
mockLoginRequest = api.loginRequest;
mockLogoutRequest = api.logoutRequest;
...
function makeUser({
  ...
  return { id, email, role, referrer_id, family_id, is_active };
}
...
describe('AuthContext', () => {
  ...
  it('login calls loginRequest and sets user in query cache', async () => {
    ...
    mockLoginRequest.mockResolvedValueOnce({ data: { user: loginUser } });
  ...
  it('logout calls logoutRequest and clears user', async () => {
    const user = makeUser({ role: 'admin' });
    mockFetchCurrentUser.mockResolvedValueOnce(user);
    mockLogoutRequest.mockResolvedValueOnce({ data: null });
  ...
  it('checkAuth on 401 retains previous user data (React Query keeps stale data on error)', async () => {
    ...
    const error = new Error('Unauthorized');
    error.response = { status: 401, data: { detail: 'Token expired' } };
    mockFetchCurrentUser.mockRejectedValueOnce(error);
