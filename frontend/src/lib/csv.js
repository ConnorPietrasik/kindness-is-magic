...
export const KNOWN_SECTIONS = ['referrers', 'families', 'people', 'users'];
 ...
 */
export function parseCsvSections(csvText) {
  const sections = {};
 ...
 */
export function validateCsvForImport(sections) {
  const errors = [];
  const warnings = [];
  const stats = {
    sections: 0,
  ...
  for (const [name, { headers, rows }] of Object.entries(sections)) {
    ...
    if (!KNOWN_SECTIONS.includes(lowerName)) {
    ...
    // Check headers
    const expected = EXPECTED_HEADERS[lowerName];
    if (!expected) {
