...
Revision ID: beb4b04634fc
Revises: 
Create Date: 2026-08-06 13:37:20.894506
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            'role',
            sa.Enum('admin', 'referrer', 'family', 'purchaser', 'delivery', name='user_role', create_constraint=True),
            nullable=False,
        ),
    ...
    op.create_index(
        op.f('ix_referrer_family_invite_code'), 'referrer', ['family_invite_code'], unique=True
    )
    ...
    op.create_foreign_key(
        'fk_users_family_id', 'users', 'family', ['family_id'], ['id'], ondelete='SET NULL'
    )
    # users.referrer_id -> referrer.id
    op.create_foreign_key(
        None, 'users', 'referrer', ['referrer_id'], ['id'], ondelete='SET NULL'
    )
    ...
    op.create_foreign_key(
        None, 'family', 'referrer', ['referrer_id'], ['id'], ondelete='SET NULL'
    )
    # family.delivery_user_id -> users.id
    op.create_foreign_key(
        'fk_family_delivery_user_id', 'family', 'users', ['delivery_user_id'], ['id'], ondelete='SET NULL'
    )
